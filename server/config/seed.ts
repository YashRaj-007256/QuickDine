...
const seedDB = async () => {
    try {
        ...
        // Seed Restaurants assigned to the owner user
        const seededRestaurants = restaurantsData.map((restaurant) => ({
            ...restaurant,
            owner: ownerUser._id,
        }));

        await Restaurant.insertMany(seededRestaurants);
        console.log("Restaurants seeded successfully.");

        console.log("Database seeded successfully!");
        console.log("-----------------------------------------");
        console.log("Default logins (password is 'password123'):");
